 ...
 * @author katie
 */
public class SFileMakerWithHierBags {
    ...
    public static void makeAllSFiles() throws FileNotFoundException{
        ...
        for(String dataset: Path.datasets){
            for(int i = 0; i<Path.nbBags; i++){
                int index = i+1;
                String pathSFile = "/Users/katie/thesiscode/datasets/multilabelUpload/"+dataset+"/settingsBag"+i+".s";
                String pathTrain = path+dataset+"/settings-bag-"+index+".arff";
                String pathTest = path+dataset+"/"+dataset+"test.arff";
                String pathHierarchy = path+dataset+"/hier"+dataset+i;
