...
import static weka.classifiers.meta.nestedDichotomies.RandomPairND.file;
...
//b-katie
  ...
  protected class NDTree implements Serializable, RevisionHandler {
    ...
    public String getRevision() {
      return RevisionUtils.extract("$Revision: 12648 $");
    }
  }
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    m_ndtree.unifyTree();
    // Build classifiers
    buildClassifierForNode(m_ndtree, data, new StringBuffer(), true);
  }
   ...
   * @param data the data to work with
     * @param stringbuffer contains the parent nodes up until now
     * @param initial is this the root node?
   * @throws Exception if anything goes wrong
   */
  public void buildClassifierForNode(NDTree node, Instances data, StringBuffer stringbuffer, boolean initial)
    throws Exception {
    ...
    int nb = data.classAttribute().numValues();
     ...
     if(initial){
          out.println();
          stringbuffer = stringbuffer.append(Arrays.toString(indsToClass(node.getIndices(), classNames)));
    ...
    if (node.m_left != null) {
      ...
      if (node.m_left.m_left != null) {
        ...
        //b-katie
        StringBuffer stringbuffer2 = new StringBuffer(stringbuffer).append(Arrays.toString(indsToClass(node.m_left.getIndices(), classNames)));
        //e-katie
        buildClassifierForNode(node.m_left, firstSubset, stringbuffer2, false);
      } //b-katie
      else {
       out.println(new StringBuffer(stringbuffer).append(Arrays.toString(indsToClass(node.m_left.getIndices(), classNames))).toString());
      }//e-katie
      if (node.m_right.m_left != null) {
        ...
        //b-katie
        StringBuffer stringbuffer2 = new StringBuffer(stringbuffer).append(Arrays.toString(indsToClass(node.m_right.getIndices(), classNames)));
        //e-katie
        buildClassifierForNode(node.m_right, secondSubset, stringbuffer2, false);
      } //b-katie
      else {
        out.println(new StringBuffer(stringbuffer).append(Arrays.toString(indsToClass(node.m_right.getIndices(), classNames))).toString());
      }//e-katie
      }
    }
