 ...
 * Nested Dichotomies with Random-Pair Selection. In: Fifteenth European
...
//b-katie	
 this.mapPath = mapPath;
    ...
    private void generateClassifierForNode(Instances data, Range classes,
            ...
            StringBuffer stringbuffer, boolean initial)
            throws Exception {

        // Get the indices van de klassen die in deze knoop aanwezig zijn
        int[] indices = classes.getSelection();

        //b-katie
        int nb = data.classAttribute().numValues();
        String[] classNames = new String[nb];
        for (int i = 0; i < nb; i++) {
            classNames[i] = data.classAttribute().value(i);
        }

        if (initial) {
            stringbuffer.append(Arrays.toString(indsToClass(indices, classNames)));
            out.println();
            //de sample data die gebruikt wordt voor de cross validation apart in een file schrijven
            //de eerste sample data lijkt de volledige data te zijn
            outData.println(new StringBuffer(data.toString()));
            outData.println();
            //de namen van de klassen in een file schrijven
        ...
        //selecteer twee klassen om mee te starten
        ...
        //voeg de initiele klassen toe aan de subsets
        ...
        if (indices.length > 2) {
            ...
            // resample the data (random subset met of zonder replacement)
            ...
            //initialIndices zijn de twee klassen die geselecteerd zijn 
            initialRwv.setNominalIndicesArr(initialIndices);

            MultiFilter multiFilter = new MultiFilter();
            //zonder de twee initiele klassen en een bepaald percentage aan samples
            ...
            //through an arbitrary filter. Like the classifier, the structure of the 
            //filter is based exclusively on the training data and test instances will 
        ...
        //set1: klasses die bij c1 behoren 
        ...
        //makeindicator: A filter that creates a new dataset with a boolean attribute 
        //replacing a nominal attribute. In the new dataset, a value of 1 is assigned 
        //to an instance that exhibits a particular range of attribute values, a 0 to
        //an instance that doesn't. The boolean attribute is coded as numeric by default.
        ...
        if (first == 1) {
            ...
            //b-katie
            StringBuffer newStringBuffer = new StringBuffer(stringbuffer).append(Arrays.toString(indsToClass(firstInds, classNames)));
            //e-katie
        ...
        if (second == 1) {
            ...
            //b-katie
            out.println(new StringBuffer(stringbuffer).append(Arrays.toString(indsToClass(secondInds, classNames))).toString());
            //e-katie
        } else {
            RemoveWithValues rwv = new RemoveWithValues();
            rwv.setInvertSelection(true);
            ...
            m_SecondSuccessor.generateClassifierForNode(secondSubset, secondRange,
                    rand, classifier, m_classifiers, new StringBuffer(newStringBuffer), false);
        }
    }
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        generateClassifierForNode(data, newRange, random, m_Classifier,
                m_classifiers, new StringBuffer(), true);
    }
    ...
    public double[] distributionForInstance(Instance inst) throws Exception {
        ...
        double[] newDist = new double[inst.numClasses()]; //per klasse een kans geven
        if (m_FirstSuccessor == null) { //er zijn geen subklassen meer, dus alle instances van hier behoren tot deze klasse?
    ...
    private String[] indsToClass(int[] indices, String[] classes) {
        String[] firstIndsString = new String[indices.length];

        for (int i = 0; i < indices.length; i++) {
            firstIndsString[i] = classes[indices[i]];
        }
        return firstIndsString;
    }
